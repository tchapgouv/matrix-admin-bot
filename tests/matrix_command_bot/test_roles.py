...
from tests.matrix_command_bot.test_simple_command import FailureCommand, SuccessCommand


@pytest.mark.asyncio
async def test_admin_role() -> None:
    roles = {USER1_ID: [Role(name="admin", all_commands=True)]}
    mocked_client, t = await create_fake_command_bot([SuccessCommand], roles=roles)
...
async def test_normal_role() -> None:
    roles = {
        USER1_ID: [
            Role(name="user1", all_commands=False, allowed_commands=[SuccessCommand])
        ],
    }
    mocked_client, t = await create_fake_command_bot([SuccessCommand], roles=roles)
    mocked_client.executed = False
    ...
    roles[USER2_ID] = [
        Role(name="user2", all_commands=False, allowed_commands=[FailureCommand])
    ]
    ...
    roles[USER2_ID] = [
        Role(name="user2", all_commands=False, allowed_commands=[SuccessCommand])
    ]
