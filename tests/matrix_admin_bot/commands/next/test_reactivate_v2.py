...
async def test_reactivate() -> None:
    ...
    # 1 call to add email
    assert len(mock_admin_client.session.request.call_args_list) == 9  # type: ignore[reportUnknownArgumentType]
    assert (
        "/users/by-username"
    ...
    assert "/user-emails" in mock_admin_client.session.request.call_args_list[6][0][1]
    assert "/reactivate" in mock_admin_client.session.request.call_args_list[7][0][1]
    assert "/user-emails" in mock_admin_client.session.request.call_args_list[8][0][1]
