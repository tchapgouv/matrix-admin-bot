...
async def test_reset_password_v2() -> None:
    ...
    mocked_matrix_client.client_session.request.side_effect = mock_requests(
        ...
        ("POST", "/set-password", mock_response_with_json(USER)),
        ("POST", "/kill-sessions", mock_response_with_json(USER)),
        (
            "GET",
...
async def test_non_local_user_reset_password_v2() -> None:
    await assert_non_local_user_rejected("!reset_password @user_to_reset:example2.org")
