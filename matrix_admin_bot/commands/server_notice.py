...
class ServerNoticeCommand(CommandWithSteps):
    ...
    async def simple_execute(self) -> bool:
        ...
        if self.state.notice_event:
            if "new_content" in self.state.notice_event.source:
                notice_content = self.state.notice_event.source["new_content"]
            else:
                notice_content = self.state.notice_event.source["content"]

            for user_id in users:
                result = result and await self.send_server_notice(
                    notice_content, user_id
                )
        else:
