...
class UnverifiedDevicesCommand(UserRelatedCommand):
    ...
    async def list_unverified_devices(self, user_id: str) -> bool:
        ...
        logger.info("Listing unverified devices", user_id=user_id)
        ...
        if res is None:
            ...
            logger.info("No device keys, nothing to report", user_id=user_id)
        ...
        if len(unverified_devices) == 0:
            logger.info("No unverified devices found", user_id=user_id)
            del self.json_report[user_id]
            return True
    ...
    async def simple_execute(self) -> bool:
        logger.info("Unverified devices - started", user_ids=self.user_ids)
