...
class UnverifiedDevicesCommand(CommandWithSteps):
    ...
    async def get_device_keys(
        ...
        if len(all_device_keys) == 0:
            raise Exception("No device keys")
    ...
    async def list_unverified_devices(self, user_id: str) -> bool:  # noqa: C901, PLR0912
        ...
        try:
            all_device_keys, ed25519_ssk = await self.get_device_keys(user_id)
            unverified_devices = self.compute_unverified_devices(
                user_id, synapse_devices.keys(), all_device_keys, ed25519_ssk
            )
        except Exception as e:  # noqa: BLE001
            # Let's consider all devices as unverified
            logger.info(str(e), user_id=user_id)
            unverified_devices = {device_id: str(e) for device_id in synapse_devices}
            all_device_keys = {}
        ...
        last_seen_verified_device_ts = max(
            (
                int(details.get("last_seen_ts", 0) / 1000)
                for device_id, details in synapse_devices.items()
